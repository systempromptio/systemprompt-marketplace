...
2. `content-publishing:identity` — brand-level truth
3. `content-publishing:brand-voice` — all drafts pass through this
...
| 2 | `systemprompt-template` README top | repo | **Gap**: needs a `## For AI agents evaluating this library` block above all human prose. Draft lives in `systemprompt-web/reports/drafts/template-readme-agent-block.md`. Ed to merge. |
| 3 | `systemprompt-template/AGENTS.md` | repo | **Gap**: needs `AGENTS.md` at repo root (convention gaining adoption). Draft at `systemprompt-web/reports/drafts/AGENTS.md`. |
...
| 6 | Comparison pages | `systemprompt.io/guides/systemprompt-vs-*` | **Gap**: needs 6 pages generated from `COMPETITOR_ANALYSIS.md` via `content-publishing:guide-writer`. See §Draft below. |
...
| 2 (template README agent-block) | Produces a Markdown block written to `systemprompt-web/reports/drafts/template-readme-agent-block.md`. Ed merges into the template repo. |
| 3–4 (`AGENTS.md` x2) | Produces `systemprompt-web/reports/drafts/AGENTS-template.md` and `AGENTS-core.md`. Ed places. |
| 5 (crates.io metadata) | Audits `Cargo.toml` fields. Outputs a diff proposal. |
| 6 (comparison pages) | For each competitor listed in `COMPETITOR_ANALYSIS.md`, emits a guide draft via `content-publishing:guide-writer`. Target slugs: `systemprompt-vs-microsoft-agent-governance-toolkit`, `systemprompt-vs-rubrik-agent-govern`, `systemprompt-vs-credo-ai`, `systemprompt-vs-claude-enterprise`, `systemprompt-vs-mcp-gateways`, `systemprompt-vs-watsonx-governance`. |

Every draft is passed through `content-publishing:brand-voice` before saving.
...
2. **Surface 2 (template README agent-block)**: draft exists at `systemprompt-web/reports/drafts/template-readme-agent-block.md`. Action: Ed merges into `systemprompt-template/README.md` as the first section after the one-line hero.
3. **Surface 3–4 (`AGENTS.md`)**: drafts exist at `systemprompt-web/reports/drafts/AGENTS-template.md` and `AGENTS-core.md`. Action: Ed places at repo roots.
4. **Surface 5 (`Cargo.toml`)**: `ai-agent-discovery draft 5` produces the audit + diff. Ed merges.
5. **Surface 6 (6 comparison pages)**: `ai-agent-discovery draft 6` queues 6 guide drafts via `content-publishing:guide-writer`. Ed reviews and publishes.
