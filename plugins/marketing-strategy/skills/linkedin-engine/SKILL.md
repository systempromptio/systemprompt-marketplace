...
Designed for daily execution via `/loop 1d marketing-strategy:linkedin-engine`.

## Dependencies (load in order)

1. `marketing-strategy:marketing-identity` — ICP, hook, channel rules
2. `content-publishing:brand-voice` — LinkedIn-specific voice, algorithm context, post structure
