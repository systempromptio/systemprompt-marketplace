...
Production cadence is daily via `/loop`, ideally after `crm:lead-tracker` completes (so we can read its output).

## Dependencies

Load `crm-identity` first (for pipeline stage definitions, scoring rubric, and exclusion list).

The `crm:lead-tracker` report for today should exist before this skill runs. Check `/var/www/html/systemprompt-web/reports/marketing/daily/{today}/lead-tracker.md`. If missing, tell Ed to run `lead-tracker` first — do not run it yourself (it has its own profile requirements).
